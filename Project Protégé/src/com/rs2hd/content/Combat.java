...
public class Combat {
	 ...
	 * @param victim The victim entity (attacked).
	 */
	...
	private static boolean canAttack(Entity source, Entity victim) {
		if((source instanceof Player) && (victim instanceof Player)) {
			...
			int wildy = Math.min(Location.wildernessLevel(pSource.getLocation()), Location.wildernessLevel(pVictim.getLocation()));
			int levelDiff = Math.abs(pSource.getSkills().getCombatLevel() - pVictim.getSkills().getCombatLevel());
			if(wildy < levelDiff) {
	 ...
	 * @param entity The entity whose combat we are handling.
	 */
